...
Public Class frmRptUmurStokSupGol
    ...
    Private Sub cmbRefreshData_Click(sender As Object, e As EventArgs) Handles cmbRefreshData.Click
        ...
        pQue = "IF OBJECT_ID('tempdb..#tmpPersediaan') IS NOT NULL DROP TABLE #tmpPersediaan " & _
                        "Select " & grpb & ",sum(b.Total) as PersediaanSekarang,sum(b.Saldo) as PersediaanSekarangQTY into #tmpPersediaan from tblGetPersediaan('" & tglAc & "') b left join vwStkSup a on a.kode = b.kode " & pQueWhereStok & " group by " & grpb & "; " & _
                        "CREATE CLUSTERED INDEX ix_tmpPersediaan ON #tmpPersediaan (" & kode & "); " & _
                    ...
                    "ctevwMstStock AS (" & _
                        ...
                        "Tahun HAVING SUM(ISNULL(a.Saldo, 0)) > 0 )," & _
                "cteBeli " & _
                ...
                "cteStock AS (" & _
                    ...
                    "((isnull(d.UmurTidakJual,999999)>=" & spHariJual.EditValue.ToString & " AND '" & cmbUmurJual.Text & "'='>=' ) OR ( isnull(d.UmurTidakJual,999999) <=" & spHariJual.EditValue.ToString & " AND '" & cmbUmurJual.Text & "'='<=' ))" & wheretrans & "), " & _
                "cte2 AS(" & _
                    "select " & kode2 & "," & _
    ...
    Private Sub mdgList_Grid_DoubleClick(sender As Object, e As EventArgs) Handles mdgList.Grid_DoubleClick
        ...
        If pformprnt.Name = "frmMainTab" Then
            If mdgList.GetRowCount_Gridview > 0 Then
                Using xx As New frmRptUmurStokSupGolDetail
                    If ceQuery.Text = "Supplier" Then
                        xx.kodesupgol = mdgList.GetRowCellValue(mdgList.FocusedRowHandle, "KdSupplier")
                    ElseIf ceQuery.Text = "Golongan" Then
                        xx.kodesupgol = mdgList.GetRowCellValue(mdgList.FocusedRowHandle, "KdGolongan")
                    End If

                    xx.ceQuery.Text = ceQuery.Text
                    xx.ceTransaksi.Text = ceTransaksi.Text
                    xx.spHariBeli.Value = spHariBeli.Value
                    xx.spHariJual.Value = spHariJual.Value
                    xx.spJual.Value = spJual.Value
                    xx.cmbUmurBeli.Text = cmbUmurBeli.Text
                    xx.cmbUmurJual.Text = cmbUmurJual.Text
                    xx.ShowDialog(Me)
                End Using
            End If
        End If
